...
export default (env, { module, plugins, output, ...config }) => ({
  ...config,
  entry: {
    'findify-react-components': path.resolve(process.cwd(), 'src/index.ts'),
    'findify-react-components.min': path.resolve(process.cwd(), 'src/index.ts'),
  },
  output: {
    ...output,
    library: 'FindifyReactComponents',
    libraryTarget: 'umd',
    umdNamedDefine: true,
  },
